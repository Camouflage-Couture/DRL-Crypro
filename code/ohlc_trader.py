...
class train:
    def __init__(self, device, algorithm, attention):
        print('using device:', device)
        self.device = device
        self.algorithm = algorithm
        self.attention = str(attention)
        self.name = algorithm + '-' + self.attention
        ...
        if self.name.__contains__('dqn'):
            ...
            # Remove 'probability' field since we're using log_prob
            self.experience = collections.namedtuple('Experience', field_names=['state', 'action', 'log_prob', 'value', 'reward', 'done'])
